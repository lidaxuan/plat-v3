...
const __STICKY_TABLE = {
  ...
  watched({ el, binding, vnode, uid }) {
    // Vue3 弃用 vnode.context，改用 vnode.component
    const instance = vnode.component
    if (!instance) return
    // 监听左侧导航栏是否折叠
    instance.watch('isNavFold', (val) => {
      instance.$nextTick(() => {
        setTimeout(() => {
          // debugger
